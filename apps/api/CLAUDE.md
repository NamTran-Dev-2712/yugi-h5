...
- Action engine-only (task 3.2): `SetSpellTrap`/`ActivateEffect` có ở engine nhưng CHƯA lên wire — `ENGINE_ONLY_ACTIONS` ở `duel-manager.ts` ẩn chúng khỏi `legalActions` và từ chối `submitAction` (`FORBIDDEN_ACTION`); `toEventView` phân loại nhưng trả `null` cho 7 event của 3.2 (không forward). Task 3.2b gỡ hai chỗ này cùng lúc với thêm `PlayerActionSchema`/`EventView`; TRƯỚC đó phải có fuzz/golden "không lộ thông tin" qua HTTP.
