...
| `POST /duels/solo` → `201`           | `{ deck?: string[], decks?: [string[], string[]], viewer?: 0 \| 1 }` (không có `deck`/`decks` = starter deck; không được gửi cả hai; key lạ bị từ chối) | `{ duelId, mode: "solo-debug", viewer, view, events, legalActions }` — `events` gồm `DuelStarted` + 10 `CardDrawn` (5 của đối thủ là lá ẩn) đã lọc cho `viewer` |
| `GET /duels/:id?viewer=0\|1` → `200` | `viewer` mặc định 0                                                                                                                                     | `{ view, legalActions }`                                                                                                                                        |
| `POST /duels/:id/actions` → `200`    | `{ playerIndex: 0 \| 1, action: { type, payload: { playerIndex, ... } } }`                                                                              | `{ view, events, legalActions }` — của ghế `playerIndex` (không bao giờ của ghế kia)                                                                            |
