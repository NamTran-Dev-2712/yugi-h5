...
Type body phản hồi (`ViewResponse`, `CreateSoloResponse`, `GetViewResponse`, `ApiErrorBody`...) ở `packages/shared/src/duel/http-types.ts` để web import. **Chưa có `legalActions`** trong `StateView`/API: client dựng nút từ state và để server từ chối (409 + `engineCode`); trang debug (`docs/design/debug-ui.md`) làm đúng như vậy.
