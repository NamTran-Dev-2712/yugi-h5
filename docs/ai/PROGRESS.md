...
- **Task tiếp theo: 2.3** (`POST /duels/solo` + `/actions` + guest token; chỉ phát `EventView`). Review P1 vẫn cần người duyệt.
- **Việc để dành (backlog P2+):** (1) `validateDeck` trong `packages/shared` (40–60 lá, ≤3 bản; dùng ở 2.3/P7); (2) TTL/dọn duel bỏ dở + store bền (DB/Redis; mutex hiện chỉ đúng 1 process); (3) Extra Deck của chủ sở hữu cho task Fusion (hiện `StateView` chỉ có `extraDeckCount`); (4) lịch sử event/resync (event chưa lưu vào session); (5) event mở đầu `StartDuel` (`DuelStarted` + 10 `CardDrawn`) chưa trả từ `createDuel`; (6) mở rộng fuzz/golden kiểm bất biến "event không lộ definitionId ẩn" khi P3 có Spell/Trap.
...
- [ ] P2 — Vertical slice: solo vs AI dummy (API + FE Duel + Sandbox) — 2.1 StateView filter: xem dưới
