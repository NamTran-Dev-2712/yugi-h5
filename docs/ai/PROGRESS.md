...
- **Task P1 còn lại** (nguồn: `docs/plan/MASTER-PLAN.md`; đọc `RULES-REVIEW-SHEET.md` + `rules-coverage.md` theo nhãn):
  ...
  7. **Flip khi bị tấn công** (task kế tiếp, số thứ tự chưa chốt) + damage step cơ bản — `[RULE]`; G6 `[DECISION]`. Đóng lỗ hổng "sân đối thủ chỉ có quái úp → không tấn công được" từ 1.6.
  8. **Hand limit 6 + `Surrender` + deck-out `DuelEnded`** — `[RULE]`; G11 `[DECISION]`. Nên tái dùng `checkLifePointsWinCondition`/pattern tương tự cho deck-out.
  9. **1.9 (số cũ)** Golden replay + fuzz harness — không có luật riêng.
...
- **Task tiếp theo: Flip khi bị tấn công** (quái úp) + damage step cơ bản; đóng lỗ hổng "sân đối thủ chỉ có quái úp → không tấn công được" từ task 1.6. Sau đó: hand limit 6, `Surrender`, deck-out phát `DuelEnded` (tái dùng `checkLifePointsWinCondition`-style helper).
