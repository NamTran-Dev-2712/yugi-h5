...
- **Task tiếp theo: lọc event theo viewer (nên làm trước 2.3)**, rồi 2.3 (`POST /duels/solo` + `/actions` + guest token). `submitAction` đang trả `events` thô — chỉ nội bộ. Review P1 vẫn cần người duyệt.
