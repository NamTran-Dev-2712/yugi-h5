...
- **Task tiếp theo: 2.6** (AI dummy `AIPlayer` random-legal-with-seed, mode `solo-vs-ai`; dùng `getLegalActions` để chọn action). **Cổng bắt buộc trước P3**: fuzz/golden kiểm bất biến "không lộ thông tin" qua HTTP khi có Spell/Trap. Review P1 vẫn cần người duyệt.
...
- [ ] P2 — Vertical slice: solo vs AI dummy (API + FE Duel + Sandbox) — 2.1, 2.2, event-filter, 2.3 xong (nháp), 2.4 (trang debug) chờ người dùng test, 2.5 (legalActions) xong (nháp); còn 2.6–2.12 (2.4 = trang debug, 2.5 = legalActions, AI dummy → 2.6, i18n → 2.12)
