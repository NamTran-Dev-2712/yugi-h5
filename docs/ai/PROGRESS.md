...
- **Task tiếp theo: 2.5** (AI dummy `AIPlayer` random-legal-with-seed, mode `solo-vs-ai`). **Cổng bắt buộc trước P3**: fuzz/golden kiểm bất biến "không lộ thông tin" qua HTTP khi có Spell/Trap. Review P1 vẫn cần người duyệt.
...
- [ ] P2 — Vertical slice: solo vs AI dummy (API + FE Duel + Sandbox) — 2.1, 2.2, event-filter, 2.3 xong (nháp), 2.4 (trang debug) chờ người dùng test; còn 2.5–2.11 (2.4 mới = trang debug, AI dummy → 2.5)
