...
## 2026-09-24 — Phaser Duel Scene (task 2.7): tách layout/presenter khỏi Phaser, theme tokens, asset manifest, fixture DEV-only

- **Đổi lộ trình P2 (chủ dự án chốt)**: 2.7 = scene tĩnh + solo-vs-ai (gồm LP/phase/log/banner, thứ MASTER-PLAN cũ chia sang 2.9/2.10), 2.8 = kéo thả + highlight (gồm SelectTribute UI, trước đó ghi 2.7), 2.9 = animation theo event + nhịp AI. Sửa đúng các dòng liên quan ở `MASTER-PLAN.md`.
- **Ba lớp thuần, không import Phaser** (`apps/web/src/duel/`): `layout.ts` (hình học khung logic 1280×720, `handSlots(n)` co khoảng cách để 0–20 lá vẫn nằm trong dải tay), `presenter.ts` (`present(StateView, legalActions, ctx) → RenderModel`), `duel-controller.ts` (nối `DuelApi`, giữ view/legalActions/log/busy). Scene chỉ vẽ `RenderModel` và chuyển click cho controller ⇒ test được bằng Vitest, đổi phong cách không đụng luật hiển thị. Presenter không bao giờ mang `definitionId` ra ngoài: lá ẩn có `label/detail = null`; thêm phòng thủ hai lớp — lá ngửa mà thuộc đối thủ và `DefenseDown` cũng bị coi là ẩn (phòng server lỗi).
- **Nút = đúng action trong `legalActions`**: `ButtonRender.action` là chính đối tượng server liệt kê (khớp `type` + `playerIndex` của viewer), không dựng payload ở client; `controller.submit` còn từ chối action không nằm trong danh sách hiện tại. "Kết thúc lượt" lặp `EndPhase` (dùng lại `shouldContinueEndTurn` của trang debug) và dừng khi prompt lấy mất `EndPhase`. Bỏ bài quá giới hạn (prompt `DiscardToHandLimit`, xảy ra ở lượt thứ 3 của người chơi nếu không đánh bài): bấm 1 lá = gửi câu trả lời 1 lá có trong `legalActions`; cần bỏ >1 lá thì chờ kéo thả (2.8), UI nói rõ.
- **Theme tokens một file** (`theme.ts`: màu, font, cỡ lá/khung, cỡ chữ) và **asset manifest** (`asset-manifest.ts`: key ổn định `board-bg`, `card-frame-monster|spell|trap`, `card-back`, `zone-slot`; nguồn `generated`). Boot sinh texture bằng Graphics theo manifest; thay ảnh thật = đổi entry sang `source:'file'`. Chuỗi UI ở `strings.ts` (tiếng Việt) để 2.12 thay bằng locale.
- **Fixture DEV-only**: `?fixture=midgame|handfull|gameover` (và 3 nút trong menu khi DEV) nạp `StateView` mẫu, không cần server. `fixture-names.ts` (chỉ tên) tách khỏi `fixtures.ts` (dữ liệu, import động sau `import.meta.env.DEV`) nên bản build production không chứa dữ liệu fixture (đã grep `dist`). Controller của fixture không có `api` ⇒ không gửi được gì.
- **Web vẫn chỉ import `@yugi/shared`**; log tái dùng `describeEvent`/`describeAiAction`/`logLinesFor` của trang debug (import, không sửa). "AI đang suy nghĩ" = đang chờ response của `EndPhase` (chỉ action này có thể chuyển lượt) `[ASSUMED]`; chưa phát lại từng bước AI (2.9).
- **Kiểm chứng**: 43 test mới; 12/12 mutant thủ công bị bắt (lần đầu 1 sống: chú thích LP đảo bên → thêm test). Screenshot dùng Edge có sẵn ở chế độ headless (không thêm dependency; Playwright không cài). Chưa chạy thật với API vì Docker tắt ở máy này — xem review packet.
  **Hệ quả:** 2.8 thêm tương tác kéo thả vào `presenter` (highlight từ `legalActions`) và scene; 2.9 tiêu thụ `aiActions` + `EventView[]` để animate, `CardRender.id = instanceId` ổn định để tái dùng object.

## 2026-09-25 — Máy trạng thái tương tác (task 2.8): thuần, luôn khớp `legalActions`, không optimistic

- **Máy trạng thái thuần** `apps/web/src/duel/interaction.ts` (`reduce(state, event, ctx) → {state, effects}`; không Phaser/I/O/đồng hồ): `idle → dragging-card | dragging-attack → choosing-option → selecting-tribute → pending-server → idle`. Effect chỉ có `send(action)` và `toast(text)`. Test được bằng dữ liệu thuần + toạ độ layout; scene chỉ chuyển pointer events và vẽ overlay. `interaction-driver.ts` (cũng thuần) nối máy với `DuelController`.
- **Luôn khớp `legalActions`**: client không dựng action, không có logic luật. `legal-index.ts` chỉ lọc/nhóm danh sách server gửi (ô hợp lệ của lá tay, tập tribute, mục tiêu tấn công, đổi thế, đáp án prompt); mọi action gửi đi là phần tử của danh sách (`emit` kiểm lại bằng so sánh cấu trúc `sameAction`, độc lập thứ tự khoá — thay `JSON.stringify` cũ ở controller). Xác nhận tribute/bỏ bài chỉ bật khi tập chọn **khớp đúng một action liệt kê** (theo ô đã thả), nên "đủ/thiếu/thừa" do server quyết, client không đếm Level. Tribute vào ô đang có quái chỉ khi server liệt kê (luật ô giải phóng 1.4).
- **Không optimistic**: board chỉ đổi khi server trả state. `pending-server` hoặc controller `busy` (AI đang đi) bỏ qua mọi input; server từ chối (409/403…) → `serverRejected` → về idle + toast, `view`/`legalActions` giữ nguyên tham chiếu (lá "về chỗ cũ" chỉ là không có gì đổi). Controller mới: `submit` trả `SubmitResult`; không liệt kê thì từ chối tại chỗ, không tới server; fixture (không API/duelId) chỉ ghi log "sẽ gửi: …".
- **Hit-test thuần ở `layout.ts`** (`pointInRect` nửa mở, `zoneIndexAt`, `optionRects`, `hitTest`: lá trên cùng theo thứ tự vẽ → LP → nút). Chuột và cảm ứng dùng chung Phaser pointer events, không listener riêng theo object (trừ hover chi tiết).
- **Bảng lỗi → tiếng Việt** ở `error-messages.ts`; test đọc `packages/game-engine/src/errors.ts` như văn bản (web không import engine) nên mã engine mới chưa có câu = test đỏ; mã lạ → câu chung + mã.
- **Bỏ bài nhiều lá tái dùng `selecting-tribute` (`purpose:'discard'`)**, tự mở khi có prompt và không có Hủy. Chọn thay vì thêm state riêng để cùng cơ chế "khớp một action liệt kê".
- **Không asset mới**: mũi tên/viền/menu/toast vẽ bằng code, token màu ở `theme.ts`. Không có ảnh tham chiếu bố cục gốc nên không đoán thêm `[GUESS]`.
- **Kiểm chứng**: viết test đỏ trước (69/147, `task-2.8-red.txt`), 19 mutant (18 bắt + 1 tương đương: guard `pending` dư vì mọi handler đã bỏ qua `pending-server`), e2e thật qua `tools/play-duel-ui.ts` (controller + máy + hit-test bằng sự kiện con trỏ, API thật; controller import không đuôi nên chạy bằng vitest e2e `apps/web/vitest.e2e.config.ts`, ngoài `pnpm test`), và `tools/ui-drag-shots.ts` (chuột thật trong Edge headless qua CDP).
  **Hệ quả:** 2.9 animate theo event; khi thêm action UI mới (Spell/Trap, kích hoạt) = thêm bộ lọc ở `legal-index.ts` + nhánh ở `interaction.ts` + test, không sửa luật. Cảm ứng thật chưa thử trên thiết bị.

## 2026-09-25 — Animation theo event (task 2.9): board giữ view cũ, khoá bằng `busy`, thời lượng ngắn hơn gốc

- **Không có state trung gian từ server** ⇒ trong lúc phát, board giữ `view` CŨ; hiệu ứng là lớp tạm vẽ chồng (`fx` container, depth 5), cuối cùng snap `response.view`. Lá mới vào sân chỉ "hiện" ở lúc snap (hiệu ứng `pop` chỉ báo trước). Chấp nhận: LP không chạy dần (số damage nổi thay thế), chưa vệt lửa/lao tới như gốc.
- **`stepsFor`/`segmentsFor` thuần** (`animation-queue.ts`): `segmentsFor` cắt `events[0:aiActions[0].eventsFrom)` cho người rồi từng `[eventsFrom,eventsTo)` cho AI (clamp, không ném lỗi), mỗi đoạn AI mở bằng step `aiLabel`. Caption = đúng câu log (`describeEvent`, tra tên lá theo view cũ, không có thì view mới) nên không lộ thêm gì. Gộp CardDrawn liên tiếp cùng người; chuỗi PhaseChanged chỉ giữ cái cuối, bỏ hẳn nếu ngay sau là TurnChanged. `DuelStarted` không có step.
- **Khoá input = tái dùng `busy`, KHÔNG thêm kind `animating` ở `interaction.ts`**: animation là trạng thái của controller (cùng nguồn với "AI đang đi"), không phải bước của gesture; `interaction.ts` đã bỏ input khi `ctx.busy`. Thêm `animating` vào `DuelUiState` chỉ để hiện gợi ý skip. `interaction.ts` không sửa dòng nào.
- **Controller**: `animator?` tiêm được (vắng = hành vi cũ nguyên vẹn: fixture/e2e/test cũ). Log ghi NGAY khi response về (debug panel giữ hành vi); animator lỗi vẫn snap và mở khoá (`try/catch`). Không animate view đầu tiên (chưa có board cũ), không animate response không có event. `endTurn` (lặp EndPhase) phát từng response trước khi gửi request kế.
- **`AnimatorHost`**: controller sinh ra trước scene và sống lâu hơn nên nhận một animator ổn định (`services.animatorHost`); scene gắn player khi hiện, gỡ khi SHUTDOWN (gỡ = skip).
- **Skip**: Space/Enter; `?fast=1` (3×), `?anim=off` (không phát gì).
- **Thời lượng (ms, `DURATION_MS`)**: draw 250 (gộp nhiều lá 600) · summon 400 · set 300 · tribute 350 · flip 400 · position 300 · attack 500 · destroy 450 · damage 600 · discard 300 · phase 250 · turn 400 · duelEnd 500 · aiLabel 250. Bảng này chỉ là bản đầu (ngắn hơn gốc 2–5× để chơi thử) — **đã thay bằng mục "Hiệu chỉnh thời lượng" ngay bên dưới**.
  **Hệ quả:** 6.1 (`EventAnimationQueue` đầy đủ, song song/VFX/audio) dựa trên `AnimationStep`/player này; thêm `EventView` mới = `tsc` đỏ ở `stepFor` cho tới khi có step.

### 2026-09-25 (tiếp) — Hiệu chỉnh `DURATION_MS` gần bản gốc (chủ dự án chốt)

Chỉ đổi bảng số + comment ở `animation-queue.ts`; không đổi logic. Nguồn: `docs/reference/notes/animation-durations.md` (video #2, mỗi mục đo **1 lần**, sai số ±0.13–0.5 s — là ước lượng, lấy điểm giữa khoảng).

| Kind           | Cũ  | Mới  | Nhãn / nguồn                                                                                                                                                                                                                |
| -------------- | --- | ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| summon         | 400 | 2600 | `[REF]` Normal Summon ~2.6 s (±0.5)                                                                                                                                                                                         |
| tribute        | 350 | 375  | `[REF]` ~0.3–0.4 s mỗi tribute (step theo từng quái, không phải cả chuỗi 2.2 s)                                                                                                                                             |
| attack         | 500 | 700  | `[REF]` thả → chạm ~0.7 s (±0.13)                                                                                                                                                                                           |
| damage         | 600 | 500  | `[REF]` số damage nổi rồi mờ ~0.5 s (LP chạy 0.65 s chưa dựng nên không cộng)                                                                                                                                               |
| destroy        | 450 | 375  | `[REF]` ~0.3–0.4 s mỗi quái                                                                                                                                                                                                 |
| turn           | 400 | 1750 | `[REF]` banner "Lượt đối thủ" ~1.7–1.8 s (±0.2); `turn` là step duy nhất ứng với banner này (`TurnChanged`)                                                                                                                 |
| phase          | 250 | 270  | `[REF]` đổi nhãn hex ~0.27 s (±0.13)                                                                                                                                                                                        |
| flip           | 400 | 400  | `[GUESS]` chưa có clip; giữ, cùng nhóm "biến đổi 1 lá" với tribute/destroy                                                                                                                                                  |
| changePosition | 300 | 350  | `[GUESS]` chỉnh nhẹ về cùng nhóm                                                                                                                                                                                            |
| discard        | 300 | 350  | `[GUESS]` cùng nhóm                                                                                                                                                                                                         |
| set            | 300 | 500  | `[GUESS]` chưa có clip; dài hơn nhóm biến đổi đơn, ngắn hơn summon (không có preview lá lớn)                                                                                                                                |
| deckOut        | 400 | 400  | `[GUESS]` giữ                                                                                                                                                                                                               |
| aiLabel        | 250 | 250  | `[GUESS]` giữ                                                                                                                                                                                                               |
| draw           | 250 | 450  | **Đơn giản hoá có chủ đích, không theo gốc**: một giá trị chung cho cả hai người; gốc chỉ đo rút của đối thủ (~1.3–1.8 s, lá bay) còn rút của mình "chưa có" (chỉ thấy số deck giảm). Gộp nhiều lá vẫn 600 (`DRAW_MANY_MS`) |
| duelEnd        | 500 | 1500 | **Đơn giản hoá**: gốc LP=0 → kết quả ~4.6 s là cả chuỗi (sóng xung kích + màn tối + loading); step của ta chỉ là một caption, banner thắng/thua có từ 2.7                                                                   |

- **Không** gán ~1700 cho `aiLabel`/`phase` dù cùng "cảm giác đổi lượt": chúng chạy **mỗi AI action** (một lượt AI ~5–8 lần) nên lượt AI sẽ kéo dài cả chục giây.
- **Tribute Summon dài hơn gốc**: `NormalSummoned` dùng chung cho Normal và Tribute Summon nên chuỗi = N×375 + 2600 (~3.3 s với 2 tribute) so với gốc ~2.2 s cả chuỗi. Tách được cần đổi logic `stepsFor` — để dành, chỉ làm nếu chủ dự án thấy chướng.
- Đo trên thực tế (`tools/ui-anim-shots.ts`, API thật): từ bấm "Kết thúc lượt" tới board ổn định ~8.5 s cho một lượt AI có 1 Normal Summon; dùng `?fast=1` (×3) khi chơi thử.

## 2026-09-25 — Log panel (task 2.10): log có cấu trúc chạy SONG SONG, không đổi `log: string[]`

- **Hướng (b)**: `DuelUiState.log` (string[]) giữ nguyên; thêm `entries: LogEntry[]` (`{text, category}`) ghi cùng chỗ (`addLog` sinh cả hai từ `entries` nên `log` = text của `entries`, cùng trần 200). Lý do: `.log` có ở 12 file (kể cả e2e thật) và trang debug dùng chung `logLinesFor`; (a) đụng hết đó để lấy một tính năng chỉ của scene. `debug-state.ts`/`debug-page.ts` không đổi dòng nào. Đổi lại có hai mảng song song: test bất biến `entries.map(text) ≡ log` sau start/action+AI/lỗi/preview fixture, và `entriesFor` ≡ `logLinesFor` (kể cả lát AI chồng lấn) chống trôi.
- **Category theo LOẠI event/action, không đoán từ text**: `categoryOfEvent`/`categoryOfAiAction` (`log-entries.ts`) switch vét cạn + `never` — thêm `EventView`/`PlayerAction` mới = `tsc` đỏ. Nhóm: **Sân** (Summon/Set/Tribute/Flip/đổi thế/discard), **Đánh** (Attack/Destroy/Damage/DuelEnded; AI: DeclareAttack/Surrender), **Lượt** (Turn/Phase/Draw/DeckOut/DuelStarted; AI: EndPhase), **Lỗi** (dòng "✗", từ controller). `[ASSUMED]`: DeckOut ở Lượt (đi cùng draw); dòng "sẽ gửi: …" (preview fixture, chỉ DEV) ở Sân; `ResolvePendingPrompt` của AI ở Sân (bỏ bài).
- **Panel là UI thuần, không qua máy trạng thái tương tác**: `log-panel.ts` (state `{visible, enabled}`, reducer, hình học suy từ `layout.log` — `layout.ts` không đổi, `logHitTest`); scene kiểm `logHitTest` ở `pointerdown` trước driver, trúng thì không chuyển tiếp. Ẩn = còn tab "Log" nhỏ ở góc phải trên cột log (x≥1032, ngoài board), không che ô/lá; phím **L**. Chip tắt = mờ; "Tất cả" sáng khi đang lọc.
- **Persist** `{visible, enabled}` ở localStorage (`yugi.logPanel`, bọc try/catch, dữ liệu hỏng/thiếu storage → mặc định hiện tất cả). Chỉ client, không gửi server.
- **Không đổi animation**: log vẫn ghi ngay khi response về (controller nguyên logic), test animation cũ xanh không sửa.
  **Hệ quả:** thêm loại event/action = thêm nhánh nhóm (`tsc` nhắc). Nếu sau này muốn trang debug cũng lọc: chuyển `DebugState.log` sang `entries` là một task riêng.
...
## 2026-09-25 — Duel Sandbox (task 2.11): dựng state ở api, session scenario có `initialState`, dev-only bằng NODE_ENV

- **Engine không đổi (chủ dự án chốt)**: engine chỉ có `StartDuel` (xáo deck, chia 5 lá), không nạp được board/phase/LP tuỳ ý. Thay vì thêm hàm vào engine, `apps/api/src/modules/dev-sandbox/scenario-to-state.ts` (hàm THUẦN) dựng `GameState` từ `Scenario`, dùng `createRng` + type `GameState` đã export. Đổi lại api phải biết quy ước của `StartDuel` (instanceId `p<seat>-<n>`, các trường mặc định): test so tập khoá của state dựng với state do `StartDuel` sinh để engine thêm trường mà quên sửa = test đỏ. Mọi action sau khi nạp vẫn qua `applyAction`.
- **`DuelSession.startAction` thành optional + `initialState?`** (chỉ session scenario có): replay bắt đầu từ `initialStateOf(session)` (`duel-store.ts`), nên session scenario **replay được** (có test) — khác dự đoán ban đầu "không replay được". 4 chỗ test cũ đổi `applyAction(null, session.startAction)` → `initialStateOf(session)` (cùng ý nghĩa).
- **`DuelManager.createDuelFromState`**: duelId = `state.matchId`; `script` (Action[] trong scenario) áp tuần tự bằng `applyAndSave` (đường duy nhất đổi state, không viết logic riêng); một bước bị engine từ chối → xoá session, trả `409 ACTION_REJECTED` "Script step N …". AI đi trước nếu scenario bắt đầu ở lượt AI (`openingResult` dùng chung với `createDuel`).
- **Schema ở `packages/shared/src/scenario/`**: `turn` là `{count, player}` (plan chỉ ghi "turn"); spell/trap có `position` (mặc định `DefenseDown` = úp, cùng quy ước StateView); `summonedTurn` bỏ trống = quái đã ở sân từ lâu (được tấn công/đổi thế ngay). Mã lỗi mới `INVALID_SCENARIO` (400; gom mọi vấn đề: id lạ, ô trùng, Spell/Trap lẫn loại). File mẫu ở `packages/shared/scenarios/*.json` (dữ liệu, không vào bundle production).
- **Dev-only**: `devOnlyModules(nodeEnv)` trả `[]` ở production ⇒ route không tồn tại ⇒ 404 (có e2e). Brief nói pattern NODE_ENV "đã có ở app.module.ts" nhưng thực tế chỉ dùng cho logger; danh sách module là tĩnh nên `AppModule` đọc `process.env['NODE_ENV']` (đã được `env.schema.ts` validate, mặc định `development`) thay vì `ConfigService`. Web: `dev/sandbox.html` không nằm trong `rollupOptions.input` ⇒ không có trong `dist`; chỉ còn 1 dòng phương thức `createSandbox` trong client HTTP dùng chung (gọi endpoint 404 ở production, vô hại).
...
## 2026-09-25 — Schema EffectDefinition (task 3.1): params phẳng theo kind, registry chỉ metadata, tên lá song ngữ

- **Params phẳng theo kind `[DECISION]`** (khác spec cũ `params: Record<string, unknown>`): mỗi kind là `z.object({kind: literal, ...field}).strict()` trong `z.discriminatedUnion('kind', …)` ở `packages/shared/src/effects/`. Gõ sai kind/field = lỗi tsc/parse. `Destroy` không có field: tác động lên `target` Card của effect. Tên kind theo bảng plan (`Damage/Heal/Draw`, không `DealDamage/DrawCard` của ví dụ cũ). `zone`/`side`/`phase` khai báo lại ở shared (không import engine); giá trị PascalCase.
- **Refine chặn effect vô nghĩa**: `operations` ≥ 1, `condition`/`cost` không rỗng nếu có, `Continuous` không `cost`/`target`, `ZoneCount` cần min/max và min ≤ max, filter ≥ 1 tiêu chí, id effect không trùng trong 1 card. Zod v3 không cho `ZodEffects` làm phần tử `discriminatedUnion`, nên refine của `ZoneCount` đặt trên cả union (`superRefine`).
- **Registry chỉ là khung**: `*_KINDS` (`satisfies` + kiểm tra hoàn chỉnh compile-time) và `OPERATION_REGISTRY: Record<OperationKind, {implemented:false}>`; không handler, không thực thi gì. Handler thật ở task 3.2.
- **Song ngữ**: `name`/`effectText?` = `{vi, en}` (cả hai bắt buộc, không fallback ngôn ngữ), helper `pickText`. Web đọc qua `duel/card-text.ts` (theo `getLang()`); trang debug/tools cố định `vi`/`en`.
- **Lệch brief (phải)**: brief nói không đụng engine, nhưng đổi shape `name` buộc sửa `summon.ts` (1 dòng message lỗi, `definition.name.en`; TS **không** báo lỗi vì template literal nhận object → sẽ in `[object Object]`, chỉ phát hiện nhờ grep) và fixture `CardDefinition` trong test/fuzz/golden của engine (`name: {vi,en}`). Không đổi logic; golden không đổi. `AttributeSchema` tách ra `cards/attribute.ts` để tránh import vòng filter ↔ card-definition (vẫn re-export từ `card-definition`).
- **`scriptId` vẫn chỉ ở mức `CardDefinition`**; ví dụ effect-level `scriptId` của spec cũ chưa hợp lệ, quyết khi có script đầu tiên.

## 2026-09-25 — SetSpellTrap + ActivateEffect (task 3.2): resolve-ngay là bước lót đường, Ignition cho Normal Spell, target qua prompt, containment ở API

- **Resolve NGAY, chưa chain** `[DECISION]`: `ActivateEffect` không đẩy vào `state.chainStack` (giữ nguyên, không định hình lại) và không có priority window. Đây là bước lót đường: **task 3.3 sẽ bọc lại bằng chain thật** (cost + target lúc activate, operations lúc resolve LIFO). Payload `ActivateEffect` được thiết kế để 3.3 không phải đổi; nếu chain buộc đổi contract thì nêu ở ADR 3.3. Hiện `prepare` (validate, không đổi state) và `execute` (cost + operations) đã tách riêng — 3.3 chỉ cần chèn chain vào giữa.
- **Tên action `SetSpellTrap`** (khớp `engine.md`, không phải `Set`). Lá úp = `position:'DefenseDown'` (quy ước fail-closed của StateView), ghi `CardInstance.setTurn` (3.4 đọc cho `trapSetTurnDelay`; 3.2 chưa đọc). Không giới hạn/lượt, không tốn quyền Normal Summon `[RULE]`. Field Spell → `NOT_ACTIVATABLE` (P4).
- **Normal Spell map vào trigger `Ignition`** `[DECISION]` (không thêm trigger kind): Spell `subType:'Normal'` ở TAY + `trigger.kind==='Ignition'`. Trap ở tay → `TRAP_NOT_SET` (C11, đọc `allowTrapActivationFromHand`); Spell khác subType hoặc trigger khác → `NOT_ACTIVATABLE`; lá đã Set trên sân chưa kích hoạt được (3.4).
- **Không có state nửa vời**: validate + mở prompt không đổi state (trừ prompt/version); trả cost + chạy operations + Spell vào mộ diễn ra trong MỘT bước cuối (`execute`). Khi trả lời prompt, engine **kiểm lại toàn bộ** activation trên state chưa đổi (không tin payload đã lưu). Lệch luật thật (cost trả trước khi chọn target) vô hại khi chưa có chain.
- **Cost bằng `costInstanceIds`** trong payload (như `tributeInstanceIds` của 1.4; không prompt cho cost), tiêu thụ tuần tự theo `cost[]`. `PayLP` cần LP **lớn hơn** số trả `[ASSUMED]` (tránh tự thua ngay khi trả cost; chưa có `[REF]`). Tribute không refactor `summon.ts` (ngoài phạm vi): viết helper `effects/costs.ts` cùng quy ước mộ (`position:null`) và tái dùng event `MonsterTributed`/`CardDiscarded`.
- **Target qua prompt `SelectEffectTarget`** (theo brief; `engine.md` cũ ghi `targetInstanceIds?` trong payload — đã sửa): đúng `count` ứng viên → tự chọn; nhiều hơn → prompt; ít hơn → `NO_VALID_TARGET`. Chỉ zone `MonsterZone`/`SpellTrapZone`. Lá úp chỉ là target khi effect **không có `filter`** `[ASSUMED]` (filter đọc danh tính lá ẩn → lộ qua legalActions).
- **Operation handler: 1 file/op** (`effects/operations/{damage,heal,draw,destroy}.ts` + `OPERATION_HANDLERS: {[K in OperationKind]: …}` — thiếu kind = `tsc` đỏ). `OPERATION_REGISTRY` ở shared chỉ `{implemented: boolean}` (đổi từ literal `false`); test engine đối chiếu hai phía. Damage tái dùng `DamageDealt` + `checkLifePointsWinCondition`; Draw tái dùng `applyDraw` (deck-out y hệt); Destroy quái → `MonsterDestroyed`, Spell/Trap → `SpellTrapDestroyed` (mới). `Destroy` không có target `Card` → `NOT_ACTIVATABLE` (effect hỏng). `version` chỉ tăng 1 lần ở cuối activation (operation không tự bump). Duel kết thúc giữa chừng → bỏ operation còn lại, `DuelEnded` luôn là event cuối.
- **Card mẫu**: chỉ **SMP-101 "Tiếp Viện Bất Ngờ"** có effect (`Ignition` + `Draw 1 self`, effectText đổi từ placeholder sang "Rút 1 lá bài"). **SMP-201 "Rào Chắn Hộ Vệ" là Trap** (chưa kích hoạt được tới 3.4, cần Negate chưa có ở batch 1) → **giữ nguyên placeholder**, không gán effect giả. Test Damage/Heal/Destroy dùng Spell định nghĩa trong test (`testing/effect-fixtures.ts`); lá thật batch 1 = task 3.8.
- **Containment ở API — 3.2 là engine-only, chưa lên wire** `[DECISION]`: brief nói không đổi api/web, nhưng event/action/mã lỗi mới làm vỡ compile/test ở đó. Chọn nhỏ nhất: (1) `toEventView` phân loại 7 event mới nhưng trả `null` (chưa forward; `never` vét cạn vẫn bắt event mới); (2) `DuelManager` lọc `SetSpellTrap`/`ActivateEffect` khỏi `legalActions` và từ chối `submitAction` (`FORBIDDEN_ACTION`) — nên HTTP, AI và web không thể tới prompt mới; `PlayerActionSchema` không đổi; (3) web chỉ thêm 8 khoá i18n `error.engine.*` (test ép mọi mã engine có câu). **Task 3.2b** (chưa đặt số cụ thể) sẽ nối wire: `PlayerActionSchema` + `EventView` + animation/log/UI kích hoạt + prompt chọn target; **cổng bắt buộc trước 3.2b**: fuzz/golden "không lộ thông tin qua HTTP" (Set Spell/Trap là lá úp đầu tiên trên sân).
- **Fuzz/golden**: fuzz sinh `SetSpellTrap`/`ActivateEffect` + trả lời prompt target (spell mẫu SP/SPK/SPP/SPD trong `FUZZ_DEFS`), 200 seed × 800 bước sạch; golden thêm case mới `spell-set-and-activate`, 5 case cũ không đổi.
- **Hệ quả:** 3.3 (chain) bọc `execute`; 3.4 đọc `setTurn` và mở `ActivateEffect` cho lá đã Set (`TRAP_SET_THIS_TURN`); 3.8 thêm lá thật. Mutation test thủ công 35 đột biến (`tools/mutants-3.2.mjs`): 35/35 bị bắt (lần đầu 30/35 → thêm test count-2 target, Discard 2, level filter, IsMyTurn, Draw đối thủ).

## 2026-09-26 — Nối wire Spell/Trap (task 3.2b): 7 event PUBLIC, lọc payload prompt, cổng fuzz không phụ thuộc shape, AI chưa dùng Spell/Trap

- **Engine 0 dòng đổi** (`git diff packages/game-engine` rỗng). Shared chỉ đổi `PlayerActionSchema` (+`SetSpellTrap`, `ActivateEffect` `.strict()`, shape lấy đúng từ `actions/types.ts`; target KHÔNG ở payload vì engine mở prompt), `EventView` (+7 type, cùng shape engine) và 2 type payload prompt (types-only). Effect schema không đổi.
- **7 event của 3.2 đều PUBLIC** `[RULE]`/`[ASSUMED]` (bảng ở `event-visibility.md`): `SpellTrapSet` không có `definitionId` (như `MonsterSet`); kích hoạt là công khai lá (`EffectActivated`/`EffectResolved`/`CardSentToGraveyard`); `SpellTrapDestroyed` đưa lá vào mộ công khai (cùng giả định với `MonsterDestroyed`); 2 event LP không có dữ liệu lá. Không cần OWNER_ONLY mới: engine đã phát event với ngữ nghĩa lộ/ẩn dựng sẵn.
- **Lọc `pendingPrompt.payload` (đổi contract nhỏ, deny by default)**: trước đây `toStateView` gửi nguyên prompt cho cả hai ghế. Payload `SelectEffectTarget` chứa `cardInstanceId` của lá trên tay đang kích hoạt (+ id cost) — không có `definitionId` nhưng cho đối thủ biết lá nào trên tay là Phép trước khi lộ. Người được hỏi thấy nguyên; người kia thấy prompt với `payload: null` trừ kind trong `PUBLIC_PROMPT_KINDS` (hiện chỉ `DiscardToHandLimit`). Kind mới mặc định bị che.
- **Cổng "không lộ definitionId" = oracle không phụ thuộc shape** (`apps/api/.../testing/leak-check.ts`): duyệt MỌI JSON một viewer nhận, mỗi object có `definitionId` phải khớp raw state sau action (mộ/banished, ngửa trên sân, hoặc lá của chính viewer trên tay/sân; lá deck không bao giờ, kể cả với chủ; `definitionId` không kèm `instanceId` = vi phạm). Chọn cách này thay vì mở rộng `cardsIn/revealed` của `event-visibility.spec.ts` (vốn biết shape) để một field mới lỡ mang danh tính lá cũng bị bắt. Chạy ở 2 tầng: `DuelManager` (`event-visibility.fuzz.spec.ts`, mặc định 8 seed × 120 bước; Spell test-only khai báo trong spec cho mọi nhánh effect: Damage/Heal/PayLP/Discard/Destroy có target) và HTTP thật (`duels.e2e.spec.ts`: golden Set SMP-201 + kích hoạt SMP-101, 3 lượt fuzz × 100 bước). Fuzz còn kiểm: ~8% lượt thử Spell/Trap sai (ghế không được hành động) phải bị từ chối và state giữ nguyên tham chiếu; phải phủ đủ 7 event + ≥1 prompt `SelectEffectTarget`.
- **AI chưa dùng Spell/Trap** (chủ dự án chốt trong brief): `chooseAction` không đổi dòng nào — nó chỉ lọc các loại action đã biết trong `legalActions` nên 2 loại mới bị bỏ qua; test mới khẳng định AI không bao giờ chọn chúng và luôn trả phần tử của `legalActions`. `STARTER_DECK` không có Spell/Trap nên luồng AI-vs-AI cũ không đổi (`play-vs-ai` 132/132). Đánh giá kích hoạt Spell là việc của task AI nâng cấp (P8).
